...
func (t *MarblesPrivateChaincode) initMarble(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	if len(marbleInput.Address) == 0 {
		return shim.Error("address field must be a non-empty string")
	}
	if len(marbleInput.Passport) == 0 {
		return shim.Error("passport field must be a non-empty string")
	}
	if len(marbleInput.NI) == 0 {
		return shim.Error("ni field must be a non-empty string")
	}
	if len(marbleInput.Creditscore) == 0 {
		return shim.Error("creditscore field must be a non-empty string")
	}
	...
	// ==== Create marble private details object with address, passport, NI and Creditscore, marshal to JSON, and save to state ====
	marblePrivateDetails := &marblePrivateDetails{
		ObjectType: "marblePrivateDetails",
		Name:       marbleInput.Name,
		Address:   marbleInput.Address,
		Passport:   marbleInput.Passport,
		NI:         marbleInput.NI,
		Creditscore:   marbleInput.Creditscore,
	}
