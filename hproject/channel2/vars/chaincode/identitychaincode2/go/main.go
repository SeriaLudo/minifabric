...
func (t *MarblesPrivateChaincode) initMarble(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	if len(marbleInput.City) == 0 {
		return shim.Error("city field must be a non-empty string")
	}
	if len(marbleInput.Dob) == 0 {
		return shim.Error("dob field must be a non-empty string")
	}
	if len(marbleInput.Age) == 0 {
		return shim.Error("age field must be a non-empty string")
	}
	if len(marbleInput.Postcode) == 0 {
		return shim.Error("postcode field must be a non-empty string")
	}
	if len(marbleInput.Owner) == 0 {
		return shim.Error("owner field must be a non-empty string")
	}
	if len(marbleInput.MHistory) == 0 {
		return shim.Error("mhistory field must be a non-empty string")
	}
	if len(marbleInput.CHI) == 0 {
		return shim.Error("chi field must be a non-empty string")
	}
	if len(marbleInput.Vaccination) == 0 {
		return shim.Error("vaccination field must be a non-empty string")
	}
	if len(marbleInput.Allergy) == 0 {
		return shim.Error("allergy field must be a non-empty string")
	}
	...
	marble := &marble{
		...
		City:      marbleInput.City,
		Dob:       marbleInput.Dob,
		Age:       marbleInput.Age,
		Postcode:  marbleInput.Postcode,
	...
	// ==== Create marble private details object with mhistory, CHI, vaccination and allergy marshal to JSON, and save to state ====
	marblePrivateDetails := &marblePrivateDetails{
		ObjectType: "marblePrivateDetails",
		Name:       marbleInput.Name,
		MHistory:   marbleInput.MHistory,
		CHI:         marbleInput.CHI,
		Vaccination: marbleInput.Vaccination,
		Allergy:     marbleInput.Allergy,
	}
	...
	//  ==== Index the marble to enable city-based range queries, e.g. return all blue marbles ====
	//  An 'index' is a normal key/value entry in state.
	//  The key is a composite key, with the elements that you want to range query on listed first.
	//  In our case, the composite key is based on indexName~city~name.
	//  This will enable very efficient state range queries based on composite keys matching indexName~city~*
	indexName := "city~name"
	cityNameIndexKey, err := stub.CreateCompositeKey(indexName, []string{marble.City, marble.Name})
	if err != nil {
		return shim.Error(err.Error())
	}
	...
	value := []byte{0x00}
	stub.PutPrivateData("collectionMarbles", cityNameIndexKey, value)
...
func (t *MarblesPrivateChaincode) delete(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	// to maintain the city~name index, we need to read the marble first and get its city
	...
	// Also delete the marble from the city~name index
	indexName := "city~name"
	cityNameIndexKey, err := stub.CreateCompositeKey(indexName, []string{marbleToDelete.City, marbleToDelete.Name})
	if err != nil {
		return shim.Error(err.Error())
	}
	err = stub.DelPrivateData("collectionMarbles", cityNameIndexKey)
	if err != nil {
		return shim.Error("Failed to delete state:" + err.Error())
	}
